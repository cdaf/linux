...
test=$(sudo cat /etc/ssh/sshd_config | grep UseDNS | grep -v '#')
if [ "$test" ]; then
...
if [ "$hypervisor" == 'hyperv' ]; then
	...
	if [ "$hypervisor" == 'virtualbox' ]; then
		if [ "$centos" ]; then
			...
			executeExpression "sudo yum install -y gcc kernel-devel-$(uname -r) kernel-headers dkms make bzip2 perl"
...
executeExpression "sudo shutdown -r 1"
